"""
smart_chatbot_deepseek.py - Enhanced chatbot system combining multiple techniques

This module provides an advanced chatbot implementation that:
1. Uses keywords for quick, reliable responses to common questions
...
PRODUCT_PATTERNS = [
    r"sách|book|sản phẩm|tác phẩm|tác giả|truyện|tiểu thuyết|tác phẩm",
    r"giá|price|cost|bao nhiêu tiền|đắt|rẻ|giá bao nhiêu|giá cả",
    r"mới|new|bán chạy|best seller|nổi tiếng|phổ biến|popular|hot|xu hướng",
    r"khuyến mãi|giảm giá|sale|discount|ưu đãi|khuyến mãi|giảm",
    r"thể loại|category|danh mục|loại sách|genre|chủ đề",
    r"tìm|find|search|kiếm|gợi ý|recommend|đề xuất",
    r"top sách|sách bán chạy|bán chạy nhất|phổ biến nhất|bestseller"  # Thêm các từ khóa về top sách
]
...
def is_product_query(user_input):
    ...
    # Sửa cách duyệt qua PRODUCT_PATTERNS vì nó là một list, không phải dictionary
    for pattern in PRODUCT_PATTERNS:  # Bỏ .keys() vì PRODUCT_PATTERNS là list
        if re.search(pattern, user_input):
            return True
    
    # Thêm xử lý đặc biệt cho các từ khóa về top sách bán chạy
    top_book_keywords = [
        'sách', 'book', 'đọc', 'tác giả', 'giá', 'cuốn', 'truyện', 'top', 'bán chạy', 
        'phổ biến', 'bestseller', 'mới nhất', 'hay nhất', 'thể loại'
    ]
    if any(keyword in user_input for keyword in top_book_keywords):
        return True
...
def format_product_info(products, max_display=3, query=None):
    ...
    if query and ("mới" in query.lower() or "new" in query.lower() or "mới nhất" in query.lower()):
        result = "Các sách mới nhất của chúng tôi:\n"
...
#     # Thay đổi phần kiểm tra greeting patterns
#     greeting_patterns = ["xin chào", "chào", "hello", "hi", "hey"]
#     # Thêm điều kiện loại trừ các từ khóa không phải lời chào
#     non_greeting_keywords = ["giới thiệu", "sách", "book", "tác giả", "giá", ":", "về"]
#     if (len(user_input_lower.split()) <= 3 and 
#         any(pattern in user_input_lower for pattern in greeting_patterns) and
#         not any(keyword in user_input_lower for keyword in non_greeting_keywords)):
...
#                 # First try to get a specific answer from DeepSeek with product context
...
#       # For more complex queries, try the DeepSeek API with conversation history
#     try:
#         # Don't format the prompt in a confusing way, just send the user's question directly
#         # and let the API use the conversation history we pass
#         api_response = get_deepseek_response(user_input, None, conversation_history if has_context else None)
        
#         if api_response and len(api_response) > 5:  # Minimal length check to ensure we got a response
#             chatbot_memory.add_message(session_id, 'bot', api_response)
#             return api_response
#         else:
#             logger.warning("Received empty or very short response from DeepSeek API")
...
def get_smart_response(user_input, products=None, session_id=None):
    ...
    # Phân tích lời chào hoặc tạm biệt
    greeting_patterns = ["xin chào", "chào", "hello", "hi", "hey"]
    is_pure_greeting = len(user_input_lower.split()) <= 2 and any(pattern in user_input_lower for pattern in greeting_patterns)

    goodbye_patterns = ["tạm biệt", "bye", "goodbye", "gặp lại sau", "hẹn gặp lại"]
    is_goodbye = any(pattern in user_input_lower for pattern in goodbye_patterns)
    ...
    # Chuẩn bị thông tin sản phẩm nếu có liên quan
    product_context = ""
    product_info = ""
    relevant_products = []

    if products and is_product_query(user_input_lower):
        try:
            relevant_products = search_products(user_input, products)
            if relevant_products:
                product_info = format_product_info(relevant_products, query=user_input)
                product_context = f"Thông tin sách liên quan:\n{product_info}\n\n"
        except Exception as e:
            logger.error(f"Error in product search: {str(e)}")

    # Ưu tiên gọi DeepSeek API
    try:
        enhanced_prompt = f"{product_context}{conversation_history if has_context else ''}\nCâu hỏi khách hàng: {user_input}\n\nHãy trả lời ngắn gọn, đầy đủ và hữu ích bằng tiếng Việt."
        logger.info(f"Sending to DeepSeek API: {enhanced_prompt[:200]}...")

        api_response = get_deepseek_response(enhanced_prompt, relevant_products if relevant_products else None, conversation_history if has_context else None)

        if api_response and len(api_response.strip()) > 20:
            logger.info(f"Got meaningful response from API: {api_response[:100]}...")
            chatbot_memory.add_message(session_id, 'bot', api_response)
            return api_response
        else:
            logger.warning(f"API response too short or empty: '{api_response}'")
    except Exception as e:
        logger.error(f"Error calling DeepSeek API: {str(e)}")

    # Fallback nếu API thất bại hoặc phản hồi không đủ
    if relevant_products:
        response = f"{product_info}\nBạn có cần thêm thông tin gì về các sản phẩm này không?"
        chatbot_memory.add_message(session_id, 'bot', response)
        return response

    # Fallback cuối cùng: keyword matching
    keyword_response = get_simple_response(user_input)
    
    # Kiểm tra xem keyword_response có nằm trong FALLBACK_RESPONSES không
    if keyword_response not in FALLBACK_RESPONSES:
        # Nếu không phải là câu trả lời mặc định, trả về ngay
        chatbot_memory.add_message(session_id, 'bot', keyword_response)
        return keyword_response
    else:
        # Nếu là câu trả lời mặc định, thêm thông báo
        fallback_message = f"{keyword_response}\n\nXin lỗi nếu câu trả lời chưa đáp ứng được câu hỏi của bạn. Bạn có thể hỏi lại với cách diễn đạt khác hoặc hỏi về chủ đề khác."
        chatbot_memory.add_message(session_id, 'bot', fallback_message)
